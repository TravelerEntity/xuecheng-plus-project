        ...
        mediaProcessList.forEach(mediaProcess -> {
            ...
            executorService.execute(()->{
                try {
                    ...
                    //上传到minio
                    String convertedObjectName = (objectName.substring(0,objectName.lastIndexOf("/")))+"/"+mp4_name;
                    boolean b1 = mediaFileService.addMediaFilesToMinIO(mp4File.getAbsolutePath(), "video/mp4", bucket, convertedObjectName);
                    if (!b1) {
                    ...
                    //mp4文件的url
                    String url = "/" + bucket + "/" + convertedObjectName;

                    //更新任务状态为成功
                    mediaFileProcessService.saveProcessFinishStatus(taskId, "2", fileId, url, "success");
                }finally {
                    //计算器减去1
